        ...
        if (cachedUser && accessToken) {
          try {
            ...
            if (parsedUser && (parsedUser.id || parsedUser._id)) {
              ...
              console.warn(
                "⚠️ Invalid cached user data, will fetch fresh data"
              );
            }
          } catch (parseError) {
...
// Extract user data - handle both response.data and response.data.user formats
const userData = response.data.user || response.data;

        // Merge with cached data to preserve any additional properties
        const cachedUser = localStorage.getItem("user");
        const mergedUserData = cachedUser
          ? { ...JSON.parse(cachedUser), ...userData }
          : userData;

        // Cache updated user data in localStorage for persistence
      ...
      if (error.status === 401 || error.status === 403) {
        ...
        console.warn(
          "Failed to verify auth, keeping cached state:",
          error.message
        );
